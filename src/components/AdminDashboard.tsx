...
import { Job, Application, PaymentSettings, AdminStats, ApplicationStatus } from '../types';
import { api } from '../lib/api';

export const AdminDashboard: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'overview' | 'jobs' | 'applicants' | 'payment_settings' | 'database_indexes'>('overview');
  ...
  const fetchAdminData = async () => {
    ...
    setErrorMsg(null);
    try {
      const [statsRes, jobsRes, appsRes, settingsRes, indexRes] = await Promise.all([
        api.getAdminStats(),
  ...
  return (
    <div className="bg-slate-950 min-h-screen text-slate-100 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        {/* VIEW 4: PAYMENT SETTINGS (PAY HERO CREDENTIALS) */}
        {activeTab === 'payment_settings' && (
          <div className="max-w-2xl mx-auto space-y-6">
            <div className="bg-slate-900 p-6 sm:p-8 rounded-3xl border border-slate-800 space-y-6">
              ...
              <div className="flex items-center gap-3 pb-4 border-b border-slate-800">
                ...
                <div>
                  <h3 className="text-xl font-black text-white">Pay Hero M-Pesa Gateway Settings</h3>
                  <p className="text-xs text-slate-400">Manage credentials for M-Pesa STK push processing</p>
                </div>
              </div>
              ...
              {settingsSaved && (
                <div className="p-3 bg-emerald-950/80 border border-emerald-500/50 rounded-xl text-emerald-300 text-xs flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 shrink-0" />
                  <span>Pay Hero credentials successfully stored in database!</span>
                </div>
              )}
              ...
              <form onSubmit={handleSavePaymentSettings} className="space-y-4">
                ...
                <div className="pt-4">
                  ...
                  >
                    <Save className="w-4 h-4" /> Update & Save Credentials
                  </button>
                </div>
